...
LABEL version=1.4.1.3e

RUN git clone http://github.com/biblepay/biblepay-evolution && \
	prefix=x86_64-pc-linux-gnu && cd biblepay-evolution/depends && make -j10 && cd .. && ./autogen.sh && ./configure --without-gui --disable-bench --disable-tests --prefix `pwd`/depends/x86_64-pc-linux-gnu && make -j10 && \
